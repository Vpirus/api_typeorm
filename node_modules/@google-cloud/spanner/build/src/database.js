...
class Database extends common.GrpcServiceObject {
    ...
    getTransaction(optionsOrCallback, callback) {
        ...
        const options = typeof optionsOrCallback === 'object' && optionsOrCallback
            ? optionsOrCallback
            : {};
        return (0, instrument_1.startTrace)('Database.getTransaction', this._traceConfig, span => {
    ...
    run(query, optionsOrCallback, cb) {
        ...
        return (0, instrument_1.startTrace)('Database.run', traceConfig, span => {
    ...
    runPartitionedUpdate(query, callback) {
        ...
        return (0, instrument_1.startTrace)('Database.runPartitionedUpdate', traceConfig, span => {
            this.pool_.getSession((err, session) => {
                if (err) {
                    (0, instrument_1.setSpanError)(span, err);
                    span.end();
    ...
    runStream(query, options) {
        ...
        return (0, instrument_1.startTrace)('Database.runStream', traceConfig, span => {
            this.pool_.getSession((err, session) => {
                if (err) {
                    (0, instrument_1.setSpanError)(span, err);
    ...
    runTransaction(optionsOrRunFn, fn) {
        ...
        const options = typeof optionsOrRunFn === 'object' && optionsOrRunFn
            ? optionsOrRunFn
            : {};
        (0, instrument_1.startTrace)('Database.runTransaction', this._traceConfig, span => {
            this.pool_.getSession((err, session, transaction) => {
                if (err) {
                    (0, instrument_1.setSpanError)(span, err);
                }
    ...
    async runTransactionAsync(optionsOrRunFn, fn) {
        ...
        return (0, instrument_1.startTrace)('Database.runTransactionAsync', this._traceConfig, async (span) => {
    ...
    batchWriteAtLeastOnce(mutationGroups, options) {
        const proxyStream = through.obj();
        return (0, instrument_1.startTrace)('Database.batchWriteAtLeastOnce', this._traceConfig, span => {
            this.pool_.getSession((err, session) => {
                if (err) {
                    proxyStream.destroy(err);
                    (0, instrument_1.setSpanError)(span, err);
