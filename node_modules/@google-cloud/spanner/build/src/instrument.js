...
function startTrace(spanNameSuffix, config, cb) {
    ...
    return getTracer((_a = config.opts) === null || _a === void 0 ? void 0 : _a.tracerProvider).startActiveSpan(SPAN_NAMESPACE_PREFIX + '.' + spanNameSuffix, { kind: api_1.SpanKind.CLIENT }, span => {
        var _a;
        span.setAttribute(semantic_conventions_1.SEMATTRS_DB_SYSTEM, 'spanner');
        ...
        if (config.tableName) {
            span.setAttribute(semantic_conventions_1.SEMATTRS_DB_SQL_TABLE, config.tableName);
        }
        if (config.dbName) {
            span.setAttribute(semantic_conventions_1.SEMATTRS_DB_NAME, config.dbName);
        }
        ...
        if (config.sql && allowExtendedTracing) {
            ...
            if (typeof sql === 'string') {
                span.setAttribute(semantic_conventions_1.SEMATTRS_DB_STATEMENT, sql);
            }
            else {
                const stmt = sql;
                span.setAttribute(semantic_conventions_1.SEMATTRS_DB_STATEMENT, stmt.sql);
            }
        }
